...
function addrows(id) {
	...
	if (checkCounter2 == 1 && editOrNot == 'no') {
	...
	if (Status_of_Client1 == 'Relabse') {
		Status_of_Client1_if_abstinent = '-';
	}
...
function editflowup(id) {
	...
	dropdownShowhideEdit('Status-of-Client1', 'Abstinent',
			'Status-of-Client1-if-abstinent-container');
...
function progDlete(id) {
	 ...
	 */
	if (confirm("The item will be deleted") == true) {
		ProgressTreat.splice(id, 1);
		for (i = 0; i <= ProgressTreat.length - 1; i++) {
			// <= equal sign add 2015-11-09
			var j = i + 1;
		...
		document
				.getElementById("progress-counsellor-observ-summery-p" + delrow).innerHTML = '';
...
function dlete(id) {
	...
	if (confirm("The item will be deleted") == true) {
		...
		for (i = 0; i <= FollowUp.length - 1; i++) {
			...
			var status_of_client_if_ab_list = document
					.getElementById('Status-of-Client1-if-abstinent');
			var rhf_fammily_list = document
					.getElementById('from-family-members');
			...
			var status_of_client_if_ab = status_of_client_if_ab_list.options[FollowUp[i][5]].value;

			var rhf_fammily = rhf_fammily_list.options[FollowUp[i][6]].value;
			var rhf_relation = rhf_relation_list.options[FollowUp[i][7]].value;
			var rhf_neighbour = rhf_neighbour_list.options[FollowUp[i][8]].value;

			var rht_fammily = rht_fammily_list.options[FollowUp[i][9]].value;
			var rht_relation = rht_relation_list.options[FollowUp[i][10]].value;
			var rht_neighbour = rht_neighbour_list.options[FollowUp[i][11]].value;

			var employment = employment_list.options[FollowUp[i][12]].value;
			var income = income_list.options[FollowUp[i][13]].value;
			...
			document.getElementById("activities-table-" + j).innerHTML = activities;
			document.getElementById("Status-of-Client-table-" + j).innerHTML = status_of_client;
			document.getElementById("Status-of-Client-if-abstinent-table-" + j).innerHTML = status_of_client_if_ab;
			document.getElementById("from-family-members-table-" + j).innerHTML = rhf_fammily;
			document.getElementById("from-relation-table-" + j).innerHTML = rhf_relation;
			document.getElementById("from-neighbour-table-" + j).innerHTML = rhf_neighbour;
			document.getElementById("to-family-members-table-" + j).innerHTML = rht_fammily;
			document.getElementById("to-relation-table-" + j).innerHTML = rht_relation;
			document.getElementById("to-neighbour-table-" + j).innerHTML = rht_neighbour;
			document.getElementById("feedback-employment-table-" + j).innerHTML = employment;
			document.getElementById("feedback-income-table-" + j).innerHTML = income;
			document.getElementById("Clients-Feedback-table-" + j).innerHTML = FollowUp[i][14];
			document.getElementById("Officers-Observations-table-" + j).innerHTML = FollowUp[i][15];

			document.getElementById("count-table-f" + j).innerHTML = j;
			document.getElementById("feed-back-date-table-f" + j).innerHTML = FollowUp[i][0];
			document.getElementById("feed-back-place-table-f" + j).innerHTML = FollowUp[i][1];
			document.getElementById("Name-of-Follow-up-Officer-table-f" + j).innerHTML = FollowUp[i][2];
			document.getElementById("activities-table-f" + j).innerHTML = activities;
			document.getElementById("Status-of-Client-table-f" + j).innerHTML = status_of_client;
			document
					.getElementById("Status-of-Client-if-abstinent-table-f" + j).innerHTML = status_of_client_if_ab;
			document.getElementById("from-family-members-table-f" + j).innerHTML = rhf_fammily;
			document.getElementById("from-relation-table-f" + j).innerHTML = rhf_relation;
			document.getElementById("from-neighbour-table-f" + j).innerHTML = rhf_neighbour;
			document.getElementById("to-family-members-table-f" + j).innerHTML = rhf_fammily;
			document.getElementById("to-relation-table-f" + j).innerHTML = rht_relation;
			document.getElementById("to-neighbour-table-f" + j).innerHTML = rht_neighbour;
			document.getElementById("feedback-employment-table-f" + j).innerHTML = employment;
			document.getElementById("feedback-income-table-f" + j).innerHTML = income;
			document.getElementById("Clients-Feedback-table-f" + j).innerHTML = FollowUp[i][14];
			document.getElementById("Officers-Observations-table-f" + j).innerHTML = FollowUp[i][15];
		}
		var delrow = FollowUp.length + 1;// remove last row
		var deleteTable = document.getElementById("followuptable");
		var deleteRowid = 'follow-table-row-' + delrow;
		var deleteMyrow = document.getElementById(deleteRowid);
		...
		document.getElementById("count-table-f" + delrow).innerHTML = '';
		document.getElementById("feed-back-date-table-f" + delrow).innerHTML = '';
		document.getElementById("feed-back-place-table-f" + delrow).innerHTML = '';
		document.getElementById("Name-of-Follow-up-Officer-table-f" + delrow).innerHTML = '';
		document.getElementById("activities-table-f" + delrow).innerHTML = '';
		document.getElementById("Status-of-Client-table-f" + delrow).innerHTML = '';
		document.getElementById("Status-of-Client-if-abstinent-table-f"
				+ delrow).innerHTML = '';
		document.getElementById("from-family-members-table-f" + delrow).innerHTML = '';
		document.getElementById("from-relation-table-f" + delrow).innerHTML = '';
		document.getElementById("from-neighbour-table-f" + delrow).innerHTML = '';
		document.getElementById("to-family-members-table-f" + delrow).innerHTML = '';
		document.getElementById("to-relation-table-f" + delrow).innerHTML = '';
		document.getElementById("to-neighbour-table-f" + delrow).innerHTML = '';
		document.getElementById("feedback-employment-table-f" + delrow).innerHTML = '';
		document.getElementById("feedback-income-table-f" + delrow).innerHTML = '';
		document.getElementById("Clients-Feedback-table-f" + delrow).innerHTML = '';
		document.getElementById("Officers-Observations-table-f" + delrow).innerHTML = '';
...
// show hide field, for edit form.

function dropdownShowhideEdit(selectid, selectedvalue, showhideid) {
	/* alert(selectid+selectedvalue+showhideid); */
	var ddl = document.getElementById(selectid);
	var selectedValue = ddl.options[ddl.selectedIndex].value;
	if (selectedValue == selectedvalue) {
	...
	/*
	 * ddl.onload = function showhide() { var selectedValue =
	 * ddl.options[ddl.selectedIndex].value;
	 *
	 * if (selectedValue == selectedvalue) {
	 * document.getElementById(showhideid).style.display = "block"; } else {
	 * document.getElementById(showhideid).style.display = "none"; } }
	 */

}

function addCounterEditForm(counterProg, counterFeed) {
	// alert('prog'+counterProg+' feed'+counterFeed);
	counter = counterProg;
	counter2 = counterFeed;
}
...
/*
 * set client Id like : CT3-F-00001 center-short-name||Gender||index 1t0 99999
 *
 */
function setClientID() {

	var gender = document.getElementById("gende");
	var selectedGender = gender.options[gender.selectedIndex].value;

	var center = document.getElementById("outrich-assigned-center");
	var selectedCenter = center.options[center.selectedIndex].value;
	// when load the page
	// geting ID
	getIDFromController(selectedCenter, selectedGender);

	// for show pregnacy part
	if (selectedGender == "Female") {
		document.getElementById("client-pregnacy").style.display = "block";
	}

	gender.onchange = function getValueGender() {
		// alert("hello gender");
		selectedGender = gender.options[gender.selectedIndex].value;
		// geting ID
		getIDFromController(selectedCenter, selectedGender);

		// for show hide pregnacy
		if (selectedGender == "Female") {
			document.getElementById("client-pregnacy").style.display = "block";
		} else {
			document.getElementById("client-pregnacy").style.display = "none";
		}
	}

	// when center drop down changing
	center.onchange = function getValueCenter() {
		// alert("hello center");
		selectedCenter = center.options[center.selectedIndex].value;
		// geting ID
		getIDFromController(selectedCenter, selectedGender);
	}

	// when gender drop down changing

}
/*
 * AJAX call for client id generationg function
 *
 */
function getIDFromController(selectedCenter, selectedGender) {
	var xmlhttp;
	...
	/*
	 * this variable add to header(2015-11-05 ); var
	 * controler_url="http://localhost/outreachmerged/index.php/Follow_Up_Form_Controller/";
	 *
	 */

	xmlhttp.onreadystatechange = function() {
		if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
			var jsonstringCount = xmlhttp.responseText;
			var array = eval("(" + jsonstringCount + ")");// convert response
			// text to ajax
			// array(responce
			// text set as JSON
			// array)
			document.getElementById("cliendid").value = array[0].ID;
		}
	}

	var params = "center_id=" + selectedCenter + "&gender=" + selectedGender;

	xmlhttp.open("POST", controler_url + "Follow_Up_Form_Controller/get_client_id", true);
...
/*
 * client Id for client editing edit page
 */
function editClientID() {
	// curently seted details
	var gender = document.getElementById("gende");
	var selectedGender = gender.options[gender.selectedIndex].value;
	var center = document.getElementById("outrich-assigned-center");
	var selectedCenter = center.options[center.selectedIndex].value;
	var currentClientID = document.getElementById("cliendid").value;
	...
	// variables for new changes
	var newGender = selectedGender;
	var newCenter = selectedCenter;
	// getIDFromController(selectedCenter,selectedGender);
	// alert(selectedGender);
	gender.onchange = function getValueGender() {
		newGender = gender.options[gender.selectedIndex].value;
		// show hide pregnacy
		if (newGender == "Female") {
			document.getElementById("client-pregnacy").style.display = "block";
		} else {
		...
		// if old gender and center not changed
		if (newCenter == selectedCenter && newGender == selectedGender) {
			document.getElementById("cliendid").value = currentClientID;

		} else {
			// if gender or center changed
			getIDFromController(newCenter, newGender);

		}
	}
	// alert(selectedCenter);
	center.onchange = function getValueCenter() {
		newCenter = center.options[center.selectedIndex].value;
		// if old gender and center not changed
		if (newCenter == selectedCenter && newGender == selectedGender) {
			document.getElementById("cliendid").value = currentClientID;
...
function dropdownShowhidePregnacy(selectid, selectedvalue, showhideid) {
	/* alert(selectid+selectedvalue+showhideid); */
	var ddl = document.getElementById(selectid);
...
// cancel editing a progress of treatement
function progCancelEdit() {
	progReset();
	progEditid = 100;// id of the edit row
	progEditOrNot = 'no';// is this edit option
	progShowOrNotNewRow = 'yes';// when submit, show new roe or show an existing
	// alert(counter);
}
// cancel editing a feedback
function feedbackCancelEdit() {
	reset();
	editid = 100;// id of the edit row
	editOrNot = 'no';// is this edit option
	showOrNotNewRow = 'yes';// when submit, show new roe or show an existing
	// alert(counter2);
}
