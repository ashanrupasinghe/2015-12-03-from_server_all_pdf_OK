	...
	function reset() {
		...
		if ($this->_checkUserEmail ( $email )) {
			...
			if ($query_status) {
				...
				$message = "<html><body>
							<div>
							<img src='" . base_url () . "asset/images/logo.png' class='img-responsive' alt='Cinque Terre' height='' width='450px'>
							</div>
							<div>
							<p>You have requested to change your password
							<br>Your new password: ".$newpw."
							<br><br>If you did not request to reset your password,
							please login to your account with this password and reset your password</p>
							</div>		
									</body></html>";
				// $email_status = mail ( $recipient, $subject, $message ); // send email
				// check email was sent or not
				$email_status=email_helper ( $sender_email, $subject, $message );
				if ($email_status) {
